    ...
    <div class="min-h-screen bg-zinc-950">
        ...
        <div v-else class="md:flex md:min-h-screen">
            ...
            <aside :class="[
                ...
                <div class="mb-4">
                    <p class="text-xs uppercase tracking-wide text-zinc-400 mb-2 px-1">Workspace</p>
                    <div class="relative">
                        ...
                        <div v-if="showWorkspaceMenu"
                            ...
                            <button v-for="ws in workspaceStore.workspaces" :key="ws.id"
                                ...
                                :class="[
                                    'w-full text-left px-3 py-2 text-sm flex items-center justify-between hover:bg-zinc-850 transition cursor-pointer',
                                    ws.id === activeWorkspace?.id ? 'text-white' : 'text-zinc-300'
                                ]">
                                <span class="truncate">{{ ws.name }}</span>
                                <span v-if="ws.id === activeWorkspace?.id" class="text-xs text-zinc-400">active</span>
                            </button>
                            ...
                            <button v-if="activeWorkspace" @click="router.push(`/workspaces/${activeWorkspace.id}/members`); showWorkspaceMenu = false; mobileNavOpen = false"
                                class="w-full text-left px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-850 transition cursor-pointer">
                                Manage workspace
                            </button>
                ...
                <nav class="space-y-1">
                    <div class="px-3 py-2 text-sm rounded-lg bg-emerald-500/10 text-emerald-400">
                        Projects
                    </div>
                </nav>
                ...
                <div class="max-w-5xl mx-auto">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
                        <div>
                            ...
                            <p v-if="activeWorkspace" class="text-sm text-zinc-400 mt-1">
                                In workspace <span class="text-zinc-300">{{ activeWorkspace.name }}</span>
                            </p>
                        </div>
                        <AppButton size="md" @click="showCreateModal = true" :disabled="!activeWorkspace"
                    ...
                    <div v-if="projectStore.projects.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <RouterLink v-for="project in projectStore.projects" :key="project.id"
                            ...
                            class="flex flex-col h-full bg-zinc-900 border border-zinc-800 rounded-lg p-5 hover:border-zinc-600 hover:bg-zinc-850/50 transition cursor-pointer group">
                            <h3 class="text-white font-semibold group-hover:text-white">{{ project.name }}</h3>
                            <p v-if="project.description" class="text-sm text-zinc-400 mt-1 line-clamp-2">{{ project.description }}</p>
                    ...
                    <div v-else class="text-center py-20 border border-dashed border-zinc-800 rounded-lg">
                        ...
                        <AppButton size="lg" @click="showCreateModal = true" :disabled="!activeWorkspace"
                            class="disabled:opacity-40">
                            Create Project
                        </AppButton>
                    </div>
        ...
        <AppModal :show="showCreateModal" title="New Project" @close="showCreateModal = false">
            <form @submit.prevent="handleCreateProject" class="space-y-4">
                <p class="text-xs text-zinc-400">In workspace <span class="text-white font-medium">{{ activeWorkspace?.name }}</span></p>
