...
The script prompts for your public URL (e.g. `https://email.example.com`); everything else is autogenerated. Supports **Ubuntu 22.04+** and **Arch-based distros** (Arch, Manjaro, EndeavourOS, CachyOS, Garuda) — for other distros use Option 2 below.

Non-interactive (CI / one-shot):

```bash
curl -fsSL https://senddock.dev/install.sh \
  | sudo SENDDOCK_PUBLIC_URL=https://email.example.com bash
```

Env overrides: `INSTALL_DIR` (default `/opt/senddock`), `SENDDOCK_PORT` (default `8080`), `SENDDOCK_PUBLIC_URL` (skips the prompt).
...
3. Fill the env vars in Dokploy's UI: `POSTGRES_PASSWORD`, `JWT_SECRET`, `PUBLIC_URL` (and `SENDDOCK_WATCHTOWER_TOKEN` if you added the override).
