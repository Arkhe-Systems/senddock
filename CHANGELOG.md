...
- **Deliverability (Pro).** A new Analytics tab: domain health (SPF/DKIM/DMARC), a per-provider breakdown with acceptance/bounce/open/click rates, and a **spam-complaint rate** ingested from an FBL/complaint webhook (`POST /webhooks/complaints/{projectId}`).
