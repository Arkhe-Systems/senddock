...
| `FRONTEND_URL` | Browser origin the dashboard is served from in a **split-frontend** setup, used for CORS headers. When set it takes precedence for CORS over the [Public URL](/guide/instance-settings#public-url); when unset, CORS falls back to the Public URL, then to `http://localhost:5173`. On the single-binary self-hosted image the dashboard is already served from the Public URL origin, so leave this unset unless you host the SPA separately. When it starts with `https://`, auth cookies are issued with `Secure: true`. | `http://localhost:5173` |
